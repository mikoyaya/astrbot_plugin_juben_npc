...
class TemplateManagementTests(unittest.TestCase):
    ...
    def test_featured_jackpot_checks_shared_library_instead_of_equipped_companion(self):
        instance = plugin_instance()
        ...
        with patch.object(PLUGIN.random, "random", return_value=0.999), patch.object(PLUGIN.random, "choice", side_effect=lambda entries: entries[0]):
        ...
        self.assertEqual(third["kind"], "大奖重复奖励转经验")
    ...
    def test_draw_rates_are_configurable_and_must_total_one_hundred(self):
        ...
        # Companion and skin probability brackets now share one jackpot
        # resolver.  This player already owns the featured companion, so both
        # branches correctly supply the missing featured skin.
        self.assertEqual(roll_at(0.50), "featured_skin")
    ...
    def test_pity_prefers_missing_current_pool_rewards_and_uses_duplicate_exp_after_completion(self):
        ...
        self.assertEqual(complete["draw_pity_state"]["pity_count"], 0)

    def test_player_assets_and_pity_are_shared_across_groups_and_pools(self):
        instance = plugin_instance()
        ...
        self.assertEqual(instance._draw_state_for(group_b, "group-b", "pool-one")["pity_count"], 9)
        self.assertEqual(instance._draw_state_for(group_b, "group-a", "pool-two")["pity_count"], 9)
        self.assertEqual(instance._draw_state_for(group_b, "group-a", "pool-one")["pity_count"], 9)

    def test_v5_migration_merges_requested_group_pity_counters(self):
        instance = plugin_instance()
        with tempfile.TemporaryDirectory() as temporary:
            directory = Path(temporary)
        ...
        self.assertEqual(instance.db["schema_version"], 5)
        ...
        self.assertEqual(shared["draw_pity_state"]["pity_count"], 22)

    def test_global_pity_migration_uses_only_the_two_requested_groups(self):
        instance = plugin_instance()
        player = {
            "npcs": {"linger": {"exp": 0}},
            "draw_pity_states": {
                "1077292821": {"default": {"pity_count": 70}},
                "642936992": {"default": {"pity_count": 50}},
                "948117623": {"default": {"pity_count": 10}},
            },
        }

        instance._migrate_global_pity(player)

        self.assertEqual(player["draw_pity_state"]["pity_count"], 99)

    def test_featured_jackpot_uses_library_ids_then_stable_exclusive_slots(self):
        instance = plugin_instance()
        featured = {
            "id": "featured", "kind": PLUGIN.COMPANION_KIND, "name": "当期同伴",
            "image": "featured.jpg", "exclusive_items": ["初版物品", "第二件"], "colors": ["#123456"],
        }
        unrelated_skin = {
            "id": "unrelated_skin", "kind": PLUGIN.SKIN_KIND, "name": "当期皮肤",
            "parent_id": "another_companion", "image": "skin.jpg", "colors": ["#123456"],
        }
        equipped = {"id": "equipped", "kind": PLUGIN.COMPANION_KIND, "name": "已装备同伴", "image": "equipped.jpg", "colors": ["#123456"]}
        instance.characters = [featured, unrelated_skin, equipped]
        player = {"npcs": {"equipped": {"exp": 0}}, "skins": {}, "items": {}, "exclusive_items": {}, "current_npc": "equipped"}

        with patch.object(PLUGIN.random, "choice", side_effect=lambda entries: entries[0]):
            first = instance._grant_featured_jackpot(player, [featured], [unrelated_skin], "大奖", "equipped")
            second = instance._grant_featured_jackpot(player, [featured], [unrelated_skin], "大奖", "equipped")
            third = instance._grant_featured_jackpot(player, [featured], [unrelated_skin], "保底", "equipped")

        self.assertEqual(first["entry_id"], "featured")
        self.assertEqual(second["entry_id"], "unrelated_skin")
        self.assertEqual(third["name"], "初版物品")
        self.assertEqual(third["image"], "featured.jpg")
        self.assertEqual(player["exclusive_items"]["featured"], {"0": {"owned_at": player["exclusive_items"]["featured"]["0"]["owned_at"]}})

        featured["exclusive_items"][0] = "改名后的物品"
        self.assertTrue(instance._has_exclusive_item(player, "featured", "改名后的物品"))
        self.assertTrue(instance._grant_exclusive_item(player, "featured"))
        self.assertIn("1", player["exclusive_items"]["featured"])

    def test_inventory_target_parser_reads_existing_player_without_creating_one(self):
        instance = plugin_instance()
        instance.db = {"schema_version": 5, "scopes": {}, "players": {"20001": instance._new_player("20001", "目标玩家")}}

        class Event:
            message_str = "同伴栏 20001 2"
        ...
        player, page = instance._inventory_view_target(Event())

        self.assertEqual(player["user_id"], "20001")
        self.assertEqual(page, 2)
        self.assertEqual(len(instance.db["players"]), 1)
    ...
    def test_inventory_keeps_four_exclusive_items_on_one_row(self):
        instance = plugin_instance()
        companions = [
            {
                "id": f"companion_{index}", "kind": PLUGIN.COMPANION_KIND, "name": f"同伴{index}",
                "english_name": "Companion", "quality": "SR", "bonus": "", "skills": [],
                "exclusive_items": ["物品甲", "物品乙", "物品丙", "物品丁"], "colors": ["#123456", "#abcdef", "#0f172a"],
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with Image.open(path) as image:
                ...
                fourth_chip_fill = image.getpixel((1175, 264))[:3]
        ...
        self.assertEqual(fourth_chip_fill, (247, 249, 252))
