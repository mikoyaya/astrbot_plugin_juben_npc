...
function createManagedObjectUrl(file) {
  const url = URL.createObjectURL(file); managedObjectUrls.add(url); return url;
}
function revokeManagedObjectUrl(url) {
  if (managedObjectUrls.delete(url)) URL.revokeObjectURL(url);
}
function clearPreviewObjectUrl(element) {
  const url = element?.dataset?.objectUrl;
  if (url) revokeManagedObjectUrl(url);
  if (element?.dataset) delete element.dataset.objectUrl;
}
function setPreviewUrl(element, url = '') {
  clearPreviewObjectUrl(element);
  if (url) element.src = url; else element.removeAttribute('src');
}
function setPreviewFile(element, file) {
  clearPreviewObjectUrl(element);
  const url = createManagedObjectUrl(file); element.dataset.objectUrl = url; element.src = url;
}
function releaseAssetPreviews(assets) {
  (assets || []).forEach((asset) => revokeManagedObjectUrl(asset?.preview));
}
window.addEventListener('beforeunload', () => [...managedObjectUrls].forEach(revokeManagedObjectUrl));
...
async function loadAll() {
  ...
  releaseAssetPreviews(checkinAssets); releaseAssetPreviews(statusAssets);
  ...
  giftPermissionIds = payload.gift_permission_ids || [];
  ...
  fillDrawDesign(drawDesign); fillGiftOperatorIds(); fillGiftPermissionIds();
...
function renderAssetOptions() {
  ...
  const exclusive = $('#grant-exclusive'); exclusive.innerHTML = '<option value="">选择同伴（按顺序发放下一件专属物品）</option>';
  characters.filter((entry) => entry.kind === 'companion').forEach((entry) => {
    const option = document.createElement('option'); option.value = entry.id; option.textContent = entry.name; exclusive.appendChild(option);
  });
}
...
function toggleKindFields(kind) {
  ...
  $$('.buyable-only').forEach((element) => element.classList.toggle('hidden', !['companion', 'skin', 'item'].includes(kind)));
  $$('.star-reward').forEach((element) => element.classList.toggle('hidden', kind !== 'companion'));
...
function fillCharacter(entry) {
  ...
  setValue(form, 'focal_x', entry.focal_x ?? 0.5); setValue(form, 'focal_y', entry.focal_y ?? 0.5); setValue(form, 'exp_amount', entry.exp_amount ?? 10); setValue(form, 'draw_weight', entry.draw_weight ?? 10); setValue(form, 'mining_weight', entry.mining_weight ?? 10); setValue(form, 'sell_price', entry.sell_price ?? 1); setValue(form, 'buy_price', entry.buy_price ?? 0); setChecked(form, 'mining_pool', entry.mining_pool); setChecked(form, 'sellable', entry.sellable); setChecked(form, 'buyable', entry.buyable); setChecked(form, 'in_pool', entry.in_pool);
  ...
  const starRewards = entry.star_rewards || {}; [2, 3, 5].forEach((level) => { const reward = starRewards[String(level)] || starRewards[level] || {}; setValue(form, `star${level}_coins`, reward.coins ?? 0); setChecked(form, `star${level}_exclusive`, reward.exclusive); });
  $('#parent-companion').value = entry.parent_id || '';
  toggleKindFields(entry.kind || 'companion');
  setPreviewUrl($('#preview-image'), entry.preview || assetUrl(entry.image)); $('#preview-title').textContent = entry.name || '未命名条目';
  $('#preview-subtitle').textContent = entry.kind === 'experience_ball'
    ? `抽中后直接 +${entry.exp_amount || 0} EXP，权重 ${entry.draw_weight || 1}`
    : entry.kind === 'item'
      ? `${kindLabel(entry.kind)} · ${entry.quality || '未标注品质'} · 抽取权重 ${entry.draw_weight || 1}${entry.sellable ? ` · 出售 ${entry.sell_price || 0} 星币` : ''}`
...
function readCharacter() {
  ...
  return {
    ...
    focal_x: numberValue(form.elements.focal_x.value, 0.5), focal_y: numberValue(form.elements.focal_y.value, 0.5), exp_amount: numberValue(form.elements.exp_amount.value, 10), draw_weight: numberValue(form.elements.draw_weight.value, 10), mining_pool: form.elements.mining_pool.checked, mining_weight: numberValue(form.elements.mining_weight.value, 10), sellable: form.elements.sellable.checked, sell_price: numberValue(form.elements.sell_price.value, 1), buyable: form.elements.buyable?.checked || false, buy_price: numberValue(form.elements.buy_price?.value, 0),
    star_rewards: Object.fromEntries([2, 3, 5].map((level) => [String(level), { coins: numberValue(form.elements[`star${level}_coins`]?.value, 0), exclusive: Boolean(form.elements[`star${level}_exclusive`]?.checked) }])),
...
async function uploadEntryFile(file) {
  ...
  $('#character-form').elements.image.value = result.image; setPreviewFile($('#preview-image'), file); toast('图片已上传，保存条目后生效。');
}
...
function fillTemplate(type, template) {
  ...
  const preview = $(`#${type}-preview`); const assetType = type === 'checkin' ? 'checkin-assets' : 'status-assets'; setPreviewUrl(preview, data.preview || assetUrl(data.background_image || data.image, assetType));
...
async function updateTemplatePreview(type, quiet = false, version = null) {
  ...
  setPreviewUrl($(`#${type}-preview`), result.preview);
...
async function uploadTemplateLayer(type, field, file) {
  ...
  const assets = type === 'checkin' ? checkinAssets : statusAssets;
  if (!assets.some((asset) => asset.filename === result.image)) assets.unshift({ filename: result.image, preview: createManagedObjectUrl(file) });
  renderTemplateAssetList(type);
...
function fillDrawDesign(design) {
  ...
  setPreviewUrl(preview, data.background_image ? assetUrl(data.background_image, 'draw-assets') : '');
}
...
function fillGiftPermissionIds() { $('#gift-permission-ids').value = giftPermissionIds.join('\n'); }
async function uploadDrawBackground(file) {
  ...
  setPreviewFile($('#draw-design-preview'), file);
...
function bindEvents() {
  ...
  $('#new-character').addEventListener('click', () => {
    ...
    toggleKindFields('companion'); setPreviewUrl($('#preview-image')); $('#preview-title').textContent = '新增条目'; $('#preview-subtitle').textContent = '填写资料后上传图片；可调画面焦点。'; renderList();
  });
  ...
  $('#grant-exclusive-button').addEventListener('click', () => safely(async () => {
    const select = $('#grant-exclusive'); const payload = { ...assetPayload(), companion_id: select.value };
    if (!payload.scope_id || !payload.user_id || !payload.companion_id) { toast('请填写群/用户并选择同伴。'); return; }
    const result = normalizeResponse(await apiPost('/grant-exclusive', payload));
    toast(result.granted_name ? `已按顺序发放：${result.granted_name}` : '已发放专属物品。'); await loadAll();
  }));
  ...
  $('#save-gift-permissions').addEventListener('click', () => safely(async () => {
    const permission_ids = $('#gift-permission-ids').value.split(/[\s,，;；]+/).map((value) => value.trim()).filter(Boolean);
    const result = normalizeResponse(await apiPost('/gift-permissions', { permission_ids }));
    giftPermissionIds = result.permission_ids || []; fillGiftPermissionIds(); toast('已保存玩家赠送权限。');
  }));
