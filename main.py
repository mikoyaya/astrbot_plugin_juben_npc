...
LEGACY_SHARED_PITY_SCOPE_IDS = {"1077292821", "642936992"}
...
FONT_DOWNLOAD_TIMEOUT_SECONDS = 10
FONT_DOWNLOAD_MAX_BYTES = 32 * 1024 * 1024
...
PERSISTENCE_MIGRATION_MARKER = ".persistent-storage-v1"
RENDER_CACHE_MAX_BYTES = 500 * 1024 * 1024
RENDER_CACHE_MAX_AGE = timedelta(days=7)
RENDER_CACHE_PROTECTED_AGE = timedelta(minutes=10)
RENDER_CACHE_PRUNE_EVERY = 25
FONT_CACHE_MAX_SIZE = 128
...
@register("astrbot_plugin_juben_npc", "Codex", "剧本杀同伴、皮肤、道具、经验球、模板、星币、打卡、挖矿与抽奖插件", "2.5.13")
class JubenNpcPlugin(Star):
    def __init__(self, context: Context):
        ...
        self.persistence_migration_marker = self.data_dir / PERSISTENCE_MIGRATION_MARKER
        ...
        self.gift_permissions_path = self.data_dir / "gift_permissions.json"
        self.db: Dict[str, Any] = {"schema_version": 5, "scopes": {}, "players": {}}
        ...
        self.gift_permission_ids: List[str] = []
    ...
    async def initialize(self):
        ...
        self.checkin_assets_dir.mkdir(parents=True, exist_ok=True)
        self.status_assets_dir.mkdir(parents=True, exist_ok=True)
        self.mining_assets_dir.mkdir(parents=True, exist_ok=True)
        self.draw_assets_dir.mkdir(parents=True, exist_ok=True)
        self.font_dir.mkdir(parents=True, exist_ok=True)
        self.render_dir.mkdir(parents=True, exist_ok=True)
        self._load_db()
        self._load_characters()
        self._migrate_all_player_records()
        ...
        self._load_gift_permissions()
        self._seed_bundled_fonts()
        self._ensure_fonts()
        self._ensure_assets()
        self._prune_render_cache(force=True)
    ...
    async def terminate(self):
        ...
        self._save_gift_permissions()

    def _migrate_legacy_data(self) -> None:
        """Copy old package-local state into AstrBot's durable plugin data area once."""
        if self.persistence_migration_marker.exists():
            return

        try:
            self.data_dir.mkdir(parents=True, exist_ok=True)
            if self.legacy_data_dir.is_dir():
                for source in self.legacy_data_dir.rglob("*"):
                    relative_path = source.relative_to(self.legacy_data_dir)
                    # Reply images are disposable cache files, not game data.
                    if relative_path.parts and relative_path.parts[0] == "rendered":
                        continue
            ...
            self.persistence_migration_marker.write_text(
                "Package-local data copied to AstrBot plugin_data. Do not remove this marker.\n",
                encoding="utf-8",
            )
        except Exception:
            # Do not mark the migration complete so a later startup can retry.
            logger.exception("迁移插件持久化数据失败，保留原目录并将在下次启动时重试。")
    ...
    async def help_cmd(self, event: AstrMessageEvent):
        path = self._render_text_card(
            ...
            [
                ...
                "购买 名称 数量 - 用星币购买后台开放的条目",
                "赠送星币 @群友 数量 - 从自己的余额转出",
                "赠送道具名 @群友 数量 - 从自己的库存转出",
                "状态栏 - 直接触发，查看当前同伴状态",
                "切换同伴 名称 - 更换当前同伴或装备皮肤",
                ...
                "道具栏 [页码] - 直接触发，查看已获得道具",
                "使用道具名 - 消耗一个普通道具",
                "出售道具名数量 - 按后台设定的单价换取星币（空格可省略）",
                "管理员：发放/扣除星币、发放道具、发放同伴、发放专属",
                "管理员：发放权限 @群友 / 取消权限 @群友",
            ],
    ...
    async def direct_command_cmd(self, event: AstrMessageEvent):
        ...
        direct_handlers = {
            ...
            "购买": self.buy_item_cmd,
            "发放星币": self.transfer_cmd,
            "扣除星币": self.deduct_coins_cmd,
            "赠送星币": self.gift_coins_cmd,
            "赠送同伴": self.grant_character_cmd,
            "发放同伴": self.grant_character_cmd,
            "赠送专属": self.grant_exclusive_cmd,
            "发放专属": self.grant_exclusive_cmd,
            "发放道具": self.grant_item_cmd,
            "发放权限": self.grant_gift_permission_cmd,
            "取消权限": self.revoke_gift_permission_cmd,
        }
        mention_first_match = re.fullmatch(r"(?:\[CQ:at,qq=\d+[^\]]*\]|@\S+)\s*(同伴栏|道具栏)(.*)", text, flags=re.I)
        if mention_first_match:
            command, suffix = mention_first_match.groups()
            if self._is_page_argument(suffix):
                handler = direct_handlers[command]
                async for result in handler(event):
                    yield result
                event.stop_event()
                return
        mention_permission_match = re.fullmatch(
            r"(?:\[CQ:at,qq=\d+[^\]]*\]|@\S+)\s*(发放权限|取消权限)\s*",
            text,
            flags=re.I,
        )
        if mention_permission_match:
            handler = direct_handlers[mention_permission_match.group(1)]
            async for result in handler(event):
                yield result
            event.stop_event()
            return
        direct_match = re.fullmatch(
            r"(发放星币|扣除星币|赠送星币|发放道具|发放同伴|赠送同伴|发放专属|赠送专属|发放权限|取消权限|购买|打卡|抽奖|状态栏|同伴栏|道具栏|挖矿|切换同伴|切换角色|更换角色|选择角色)(.*)",
            text,
        )
        if direct_match:
            ...
            if command in {"打卡", "抽奖", "挖矿"}:
                ...
                valid = self._is_inventory_argument(suffix)
            elif command in {"切换同伴", "切换角色", "更换角色", "选择角色"}:
                valid = bool(suffix.strip())
            elif command in {"购买", "发放道具", "发放同伴", "赠送同伴", "发放专属", "赠送专属", "发放星币", "扣除星币", "赠送星币", "发放权限", "取消权限"}:
                valid = bool(suffix.strip())
            if valid:
                handler = direct_handlers[command]
                async for result in handler(event):
        ...
        if re.fullmatch(r"出售(?:道具)?\s*\S.*", text):
            async for result in self.sell_item_cmd(event):
                yield result
            event.stop_event()
            return

        if re.fullmatch(r"赠送道具(?:\s+)?\S.*", text):
            async for result in self.gift_item_cmd(event):
                yield result
            event.stop_event()
            return

        if self._is_player_item_gift_message(event, text):
            async for result in self.gift_item_cmd(event):
                yield result
            event.stop_event()
            return
    ...
    @filter.command("发放星币", alias={"给星币", "加星币"})
    async def transfer_cmd(self, event: AstrMessageEvent):
        ...
        if not target_id or amount <= 0:
            path = self._render_text_card("发放失败", ["格式：发放星币 @群友 数量", "例如：发放星币 @小明 20"])
            yield event.image_result(str(path))
            return
    ...
    @filter.command("赠送星币")
    async def gift_coins_cmd(self, event: AstrMessageEvent):
        """Transfer coins from a permitted player's balance to another player."""
        if not self._has_gift_permission(self._sender_id(event)):
            yield event.image_result(str(self._render_notice_card("赠送失败", "你没有赠送权限，请联系管理员在后台授权。")))
            return
        target_id, target_label, amount = self._parse_transfer(event)
        if not target_id or amount <= 0:
            yield event.image_result(str(self._render_text_card("赠送失败", ["格式：赠送星币 @群友 数量", "例如：赠送星币 @小明 20"])))
            return
        sender_id = self._sender_id(event)
        if str(target_id) == str(sender_id):
            yield event.image_result(str(self._render_notice_card("赠送失败", "不能将星币赠送给自己。")))
            return
        sender = self._get_player(event)
        if int(sender.get("coins", 0) or 0) < amount:
            yield event.image_result(str(self._render_notice_card("赠送失败", f"你的星币不足，需要 {amount}，当前只有 {sender.get('coins', 0)}。")))
            return
        recipient = self._existing_player_by_scope(self._scope_id(event), target_id)
        if recipient is None:
            yield event.image_result(str(self._render_notice_card("赠送失败", "该成员尚未出现在游戏数据库中。")))
            return
        sender["coins"] = max(0, int(sender.get("coins", 0) or 0) - amount)
        recipient["coins"] = max(0, int(recipient.get("coins", 0) or 0)) + amount
        self._save_db()
        yield event.image_result(str(self._render_compact_notice_card(
            "星币已赠送", [f"对象：{target_label}", f"赠送：{amount} 星币", f"你的余额：{sender['coins']} 星币"]
        )))

    @filter.command("扣除星币", alias={"扣星币", "扣除星币数"})
    async def deduct_coins_cmd(self, event: AstrMessageEvent):
        denial = await self._operator_denial(event, "扣除星币")
        if denial:
            yield event.image_result(str(denial))
            return
        ...
        target_id, target_label, amount = self._parse_transfer(event)
        if not target_id or amount <= 0:
            path = self._render_text_card("扣除失败", ["格式：扣除星币 @群友 数量", "例如：扣除星币 @小明 20"])
            yield event.image_result(str(path))
            return

        target = self._get_player_by_id(event, target_id, target_label)
        before = max(0, int(target.get("coins", 0) or 0))
        removed = min(before, amount)
        target["coins"] = max(0, before - removed)
        self._save_db()
        logger.info(
            "剧本杀星币扣除：operator=%s target=%s requested=%s removed=%s balance=%s",
            self._sender_id(event), target_id, amount, removed, target["coins"],
        )
        path = self._render_compact_notice_card(
            "星币已扣除",
            [f"对象：{target_label}", f"实际扣除 -{removed} 星币", f"当前余额：{target['coins']} 星币"],
        )
        yield event.image_result(str(path))

    @filter.command("赠送同伴", alias={"发放同伴", "赠送角色", "赠送NPC", "赠送皮肤", "给角色"})
    ...
    async def grant_exclusive_cmd(self, event: AstrMessageEvent):
        ...
        if not target_id or not companion or companion.get("kind") != COMPANION_KIND:
            yield event.image_result(str(self._render_notice_card("赠送失败", "格式：发放专属 @群友 同伴名")))
            return
        target = self._get_player_by_id(event, target_id, target_label)
    ...
    @filter.command("发放权限")
    async def grant_gift_permission_cmd(self, event: AstrMessageEvent):
        denial = await self._operator_denial(event, "发放权限")
        if denial:
            yield event.image_result(str(denial))
            return
        target_id, target_label = self._parse_target_user(event)
        if not target_id:
            yield event.image_result(str(self._render_notice_card("操作失败", "格式：发放权限 @群友")))
            return
        target_id = str(target_id)
        if target_id not in self.gift_permission_ids:
            self.gift_permission_ids.append(target_id)
            self._save_gift_permissions()
            result = "已获得赠送星币和赠送道具权限"
        else:
            result = "原本已有赠送权限"
        yield event.image_result(str(self._render_notice_card("权限处理完成", f"对象：{target_label}，{result}。")))

    @filter.command("取消权限")
    async def revoke_gift_permission_cmd(self, event: AstrMessageEvent):
        denial = await self._operator_denial(event, "取消权限")
        if denial:
            yield event.image_result(str(denial))
            return
        target_id, target_label = self._parse_target_user(event)
        if not target_id:
            yield event.image_result(str(self._render_notice_card("操作失败", "格式：取消权限 @群友")))
            return
        before = len(self.gift_permission_ids)
        self.gift_permission_ids = [value for value in self.gift_permission_ids if value != str(target_id)]
        self._save_gift_permissions()
        result = "已取消" if len(self.gift_permission_ids) != before else "该用户原本没有赠送权限"
        yield event.image_result(str(self._render_notice_card("权限处理完成", f"对象：{target_label}，{result}。")))
    ...
    async def inventory_cmd(self, event: AstrMessageEvent):
        player, page = self._inventory_view_target(event)
        if player is None:
            yield event.image_result(str(self._render_notice_card("查询失败", "没有找到这个 QQ 的玩家档案。")))
            return
        path = self._render_inventory(player, page)
        yield event.image_result(str(path))
    ...
    async def item_inventory_cmd(self, event: AstrMessageEvent):
        ...
        if player is None:
            yield event.image_result(str(self._render_notice_card("查询失败", "没有找到这个 QQ 的玩家档案。")))
            return
        path = self._render_item_inventory(player, page)
        yield event.image_result(str(path))
    ...
    @filter.command("出售", alias={"出售道具"})
    async def sell_item_cmd(self, event: AstrMessageEvent):
        item, amount = self._parse_item_amount_command(event, "出售")
        if not item or amount is None:
            yield event.image_result(
                str(self._render_notice_card("出售失败", "格式：出售道具名数量，例如：出售回复药剂2（空格也可用）"))
            )
            return
        if not self._template_bool(item.get("sellable", False)):
            yield event.image_result(str(self._render_notice_card("出售失败", f"{item['name']} 当前不可出售。")))
            return
        price = int(item.get("sell_price", 0) or 0)
        if not 1 <= price <= 1_000_000:
            yield event.image_result(str(self._render_notice_card("出售失败", f"{item['name']} 尚未配置有效出售单价。")))
            return

        player = self._get_player(event)
        result = self._consume_item(player, item["id"], amount)
        if not result["removed"]:
            yield event.image_result(
                str(self._render_notice_card("出售失败", f"你的道具栏中 {item['name']} 数量不足。"))
            )
            return
        ...
        self._save_db()
        logger.info(
            "剧本杀道具出售：user=%s scope=%s item=%s amount=%s coins=%s balance=%s",
            self._sender_id(event), self._scope_id(event), item["id"], amount, coins, player["coins"],
        )
        yield event.image_result(
            str(
                self._render_compact_notice_card(
                    "道具已出售",
                    [f"道具：{item['name']} x{amount}", f"获得：+{coins} 星币", f"当前余额：{player['coins']} 星币"],
                )
            )
        )
    ...
    async def buy_item_cmd(self, event: AstrMessageEvent):
        entry, amount = self._parse_purchase_command(event)
        if not entry or amount is None:
            yield event.image_result(str(self._render_notice_card("购买失败", "格式：购买 道具/同伴/皮肤名称 数量")))
            return
        if not self._template_bool(entry.get("buyable", False)):
            yield event.image_result(str(self._render_notice_card("购买失败", f"{entry['name']} 当前不可购买。")))
            return
        kind = entry.get("kind")
        if kind in {COMPANION_KIND, SKIN_KIND} and amount != 1:
            yield event.image_result(str(self._render_notice_card("购买失败", "同伴和皮肤只能一次购买 1 个。")))
            return
        price = int(entry.get("buy_price", 0) or 0)
        total = price * amount
        if price < 1 or total < 1:
            yield event.image_result(str(self._render_notice_card("购买失败", "该条目尚未配置有效购买价格。")))
            return
        player = self._get_player(event)
        if int(player.get("coins", 0) or 0) < total:
            yield event.image_result(str(self._render_notice_card("购买失败", f"星币不足，需要 {total}，当前只有 {player.get('coins', 0)}。")))
            return

        exclusive_name = ""
        if kind == SKIN_KIND and entry["id"] in player.get("skins", {}):
            yield event.image_result(str(self._render_notice_card("购买失败", "你已经拥有这款皮肤，不能重复购买。")))
            return
        if kind == COMPANION_KIND and entry["id"] in player.get("npcs", {}):
            names = self._exclusive_item_names(entry)
            owned = self._owned_exclusive_slots(player, entry["id"])
            next_slot = next((index for index in range(len(names)) if index not in owned), None)
            if next_slot is None:
                yield event.image_result(str(self._render_notice_card("购买失败", "该同伴的四个专属物品已经全部拥有。")))
                return
            exclusive_name = names[next_slot]

        player["coins"] = max(0, int(player.get("coins", 0) or 0) - total)
        if kind == COMPANION_KIND and exclusive_name:
            granted = self._grant_exclusive_item(player, entry["id"], exclusive_name)
            result_label = f"专属物品：{exclusive_name}" if granted else "专属物品发放失败"
        elif kind == ITEM_KIND:
            for _ in range(amount):
                self._grant_character(player, entry["id"])
            result_label = f"{entry['name']} x{amount}"
        else:
            self._grant_character(player, entry["id"])
            result_label = entry["name"]
        self._save_db()
        yield event.image_result(str(self._render_compact_notice_card(
            "购买成功", [f"内容：{result_label}", f"消耗：{total} 星币", f"剩余余额：{player['coins']} 星币"]
        )))

    async def gift_item_cmd(self, event: AstrMessageEvent):
        if not self._has_gift_permission(self._sender_id(event)):
            yield event.image_result(str(self._render_notice_card("赠送失败", "你没有赠送道具权限，请联系管理员在后台授权。")))
            return
        target_id = self._extract_at_id(event)
        item, amount = self._parse_item_amount_command(event, "赠送", requires_target=True)
        if not target_id or not item or amount is None:
            yield event.image_result(
                str(self._render_notice_card("赠送失败", "格式：赠送道具名 @群友 数量，例如：赠送回复药剂 @小明 2"))
            )
            return

        sender_id = self._sender_id(event)
        if str(target_id) == str(sender_id):
            yield event.image_result(str(self._render_notice_card("赠送失败", "不能将道具赠送给自己。")))
            return

        scope_id = self._scope_id(event)
        recipient = self._existing_player_by_scope(scope_id, target_id)
        if not recipient:
            yield event.image_result(str(self._render_notice_card("赠送失败", "该成员尚未出现在游戏数据库中。")))
            return

        sender = self._get_player(event)
        result = self._consume_item(sender, item["id"], amount)
        if not result["removed"]:
            yield event.image_result(
                str(self._render_notice_card("赠送失败", f"你的道具栏中 {item['name']} 数量不足。"))
            )
            return

        recipient_item = recipient.setdefault("items", {}).setdefault(
            item["id"], {"count": 0, "owned_at": datetime.now().strftime("%Y-%m-%d")}
        )
        recipient_item["count"] = int(recipient_item.get("count", 0) or 0) + amount
        self._save_db()
        logger.info(
            "剧本杀道具赠送：sender=%s scope=%s recipient=%s item=%s amount=%s coins=0",
            sender_id, scope_id, target_id, item["id"], amount,
        )
        yield event.image_result(
            str(
                self._render_compact_notice_card(
                    "道具已赠送",
                    [f"对象：用户{target_id}", f"道具：{item['name']} x{amount}", f"剩余：{result['remaining']} 个"],
                )
            )
        )

    @filter.command("发放道具")
    async def grant_item_cmd(self, event: AstrMessageEvent):
        denial = await self._operator_denial(event, "发放道具")
        if denial:
            yield event.image_result(str(denial))
            return
        target_id, target_label = self._parse_target_user(event)
        item, amount = self._parse_item_amount_command(event, "发放", requires_target=True)
        if not target_id or not item or amount is None:
            yield event.image_result(str(self._render_notice_card("发放失败", "格式：发放道具 @群友 道具名 数量")))
            return
        target = self._get_player_by_id(event, target_id, target_label)
        target_item = target.setdefault("items", {}).setdefault(
            item["id"], {"count": 0, "owned_at": datetime.now().strftime("%Y-%m-%d")}
        )
        target_item["count"] = max(0, int(target_item.get("count", 0) or 0)) + amount
        self._save_db()
        yield event.image_result(str(self._render_compact_notice_card(
            "道具已发放", [f"对象：{target_label}", f"道具：{item['name']} x{amount}", f"当前数量：{target_item['count']}" ]
        )))
    ...
    def _register_page_apis(self, context: Context):
            ...
            return jsonify(
                {
                    ...
                    "gift_permission_ids": self.gift_permission_ids,
                    "checkin_templates": [
                        {
                            **template,
        ...
        async def save_character():
            ...
            if raw_kind == ITEM_KIND and self._template_bool(data.get("sellable", False)):
                if self._sale_price_or_none(data.get("sell_price")) is None:
                    return jsonify({"status": "error", "message": "可出售道具的单价必须是 1 到 1000000 的整数。"}), 400
        ...
        async def save_gift_permissions():
            data = (await request.get_json()) or {}
            permission_ids, invalid_ids = self._parse_gift_operator_ids(data.get("permission_ids", data.get("user_ids", [])))
            if invalid_ids:
                return jsonify({"status": "error", "message": f"赠送权限名单包含无效 QQ：{', '.join(invalid_ids[:5])}"}), 400
            self.gift_permission_ids = permission_ids
            self._save_gift_permissions()
            return jsonify({"ok": True, "permission_ids": self.gift_permission_ids})
        ...
        async def grant_exclusive_item():
            ...
            before_slots = self._owned_exclusive_slots(player, companion_id)
            ...
            next_slot = next((index for index in range(len(names)) if index not in before_slots), None)
            self._save_db()
            return jsonify({"ok": True, "created": True, "granted_name": names[next_slot] if next_slot is not None and next_slot < len(names) else "专属物品", "player": player})
        ...
        context.register_web_api(f"/{PLUGIN_NAME}/gift-permissions", save_gift_permissions, ["POST"], "Save player gift permissions")
    ...
    def _migrate_database(self):
        ...
        for scope in scopes.values():
            ...
            for player in players.values():
                ...
                player.setdefault("star_rewards_claimed", {})
        ...
        # Version 4 made a QQ user's assets global while keeping the prior
        # per-group pity records for compatibility.  Version 5 below merges
        # the two requested legacy group counters into one global counter.
        ...
        # Version 5 makes the guarantee counter a player-level value.  Only
        # the two groups selected by the operator are carried forward from the
        # previous per-group design; all legacy group records are discarded.
        if previous_version < 5:
            self._backup_once(self.db_path, "v4")
            for player in shared_players.values():
                if not isinstance(player, dict):
                    continue
                self._migrate_global_pity(player)
            self.db["schema_version"] = 5
            self._save_db()

    @staticmethod
    def _pity_number(state: Any) -> int:
        if not isinstance(state, dict):
            return 0
        try:
            return max(0, int(state.get("pity_count", 0) or 0))
        except (TypeError, ValueError):
            return 0

    def _migrate_global_pity(self, player: Dict[str, Any]) -> None:
        """Merge the two requested legacy group counters into one counter."""
        scoped = player.get("draw_pity_states")
        merged: Dict[str, Any] = {}
        total = 0
        if isinstance(scoped, dict):
            for scope_id, scope_states in scoped.items():
                if not isinstance(scope_states, dict):
                    continue
                for state in scope_states.values():
                    if not isinstance(state, dict):
                        continue
                    value = self._pity_number(state)
                    if str(scope_id) in LEGACY_SHARED_PITY_SCOPE_IDS:
                        total += value
                        if not merged:
                            merged = copy.deepcopy(state)
        if not isinstance(merged, dict):
            merged = {}
        # When neither selected group has a record, the old group-only value
        # is intentionally discarded instead of leaking into global pity.
        if not total:
            merged = {}
        merged["pity_count"] = 99 if total > DRAW_PITY_TARGET else max(0, total)
        merged.pop("guarantee_stage", None)
        merged.setdefault("next_pity_kind", "random")
        merged.setdefault("starter_pending", not bool(player.get("npcs")))
        merged.setdefault("starter_skin_pending", False)
        player["draw_pity_state"] = merged
        player["draw_state"] = merged
        # The old keys contained per-group pity counters.  Keeping them after
        # the one-time conversion is misleading and risks accidental reuse by
        # older integrations, so remove them entirely.
        player.pop("draw_pity_states", None)

    def _migrate_all_player_records(self) -> None:
        """Canonicalize legacy player fields after the character library loads."""
    ...
    def _load_gift_permissions(self):
        self.gift_permission_ids = []
        if not self.gift_permissions_path.exists():
            self._save_gift_permissions()
            return
        try:
            raw = json.loads(self.gift_permissions_path.read_text(encoding="utf-8"))
        except Exception as exc:
            logger.error(f"读取玩家赠送权限失败，将默认禁用：{exc}")
            raw = {}
        values = raw.get("permission_ids", raw.get("user_ids", [])) if isinstance(raw, dict) else raw
        self.gift_permission_ids, invalid_ids = self._parse_gift_operator_ids(values)
        if invalid_ids:
            logger.warning("玩家赠送权限名单包含无效项，已忽略：%s", ", ".join(invalid_ids))
        self._save_gift_permissions()

    def _save_gift_permissions(self):
        self.data_dir.mkdir(exist_ok=True)
        self._write_json_atomic(self.gift_permissions_path, {"permission_ids": self.gift_permission_ids})

    def _has_gift_permission(self, user_id: str) -> bool:
        return str(user_id or "") in set(self.gift_permission_ids) or str(user_id or "") in set(self.gift_operator_ids)
    ...
    @staticmethod
    def _sale_price_or_none(value: Any) -> Optional[int]:
        """Accept only the configured integer sale-price range without clamping."""
        ...
        try:
            ...
            return None
        if not price.is_integer() or not 1 <= price <= 1_000_000:
            return None
        return int(price)
    ...
    @staticmethod
    def _normalize_star_rewards(value: Any) -> Dict[str, Dict[str, Any]]:
        """Normalize one-time rewards for the 2/3/5 star milestones."""
        source = value if isinstance(value, dict) else {}
        normalized: Dict[str, Dict[str, Any]] = {}
        for level in STAR_REWARD_LEVELS:
            raw = source.get(str(level), source.get(level, {}))
            raw = raw if isinstance(raw, dict) else {}
            try:
                coins = max(0, min(1_000_000_000, int(raw.get("coins", 0) or 0)))
            except (TypeError, ValueError):
                coins = 0
            exclusive = str(raw.get("exclusive", raw.get("grant_exclusive", False))).lower() in {"1", "true", "yes", "on"}
            normalized[str(level)] = {"coins": coins, "exclusive": exclusive}
        return normalized
    ...
    def _normalize_character(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if kind == ITEM_KIND:
            sellable = self._template_bool(data.get("sellable", False))
            sell_price = self._sale_price_or_none(data.get("sell_price")) or 0
            if not sellable:
                sell_price = 0
            return {
                "id": character_id,
                ...
                "buyable": self._template_bool(data.get("buyable", False)),
                "buy_price": max(0, int(self._template_number(data.get("buy_price"), 0, 0, 1_000_000_000))),
        ...
        return {
            ...
            "star_rewards": self._normalize_star_rewards(data.get("star_rewards")),
            "buyable": self._template_bool(data.get("buyable", False)),
            "buy_price": max(0, int(self._template_number(data.get("buy_price"), 0, 0, 1_000_000_000))),
            "in_pool": in_pool,
            "featured": in_pool,
    ...
    def _new_player(self, user_id: str, name: str) -> Dict[str, Any]:
        return {
            ...
            "draw_pity_state": {"pity_count": 0, "next_pity_kind": "random", "starter_pending": True, "starter_skin_pending": False},
        }
    ...
    def _get_player_by_scope(self, scope_id: str, user_id: str, name: str) -> Dict[str, Any]:
        ...
        player.setdefault("draw_pity_state", copy.deepcopy(player.get("draw_state", {})))
        self._migrate_player_npcs(player)
    ...
    def _migrate_player_npcs(self, player: Dict[str, Any]):
        ...
        for companion_id, value in list(exclusive_items.items()):
            ...
            names = self._exclusive_item_names(companion)
            # v5 uses stable slot IDs ("0", "1", ...), while older saves
            # used display names.  Preserve acquisition order for entries that
            # were renamed in the library.
            if isinstance(value, str):
                exclusive_items[companion_id] = {"0": {"owned_at": value}}
                continue
            if not isinstance(value, dict):
                exclusive_items[companion_id] = {"0": {"owned_at": ""}}
                continue
            ...
            if "owned_at" in value:
                exclusive_items[companion_id] = {"0": {"owned_at": str(value.get("owned_at") or "")}}
                continue
            if all(str(key).isdigit() for key in value):
                exclusive_items[companion_id] = {
                    str(key): {"owned_at": str(record.get("owned_at") or "")} if isinstance(record, dict) else {"owned_at": str(record or "")}
            ...
            for item_name, record in value.items():
                ...
                try:
                    ...
                    while str(next_slot) in normalized_owned:
                        next_slot += 1
                    slot = next_slot
                    next_slot += 1
                if isinstance(record, str):
                    normalized_owned[str(slot)] = {"owned_at": record}
                elif isinstance(record, dict):
                    normalized_owned[str(slot)] = {"owned_at": str(record.get("owned_at") or "")}
            exclusive_items[companion_id] = normalized_owned

        claimed_rewards = player.setdefault("star_rewards_claimed", {})
        if not isinstance(claimed_rewards, dict):
            player["star_rewards_claimed"] = claimed_rewards = {}
        # Existing EXP is historical data.  Mark milestones already reached as
        # claimed so adding a new configuration later cannot retroactively pay
        # old players; only a future level crossing can trigger a reward.
        for companion_id, record in npcs.items():
            if not isinstance(record, dict):
                continue
            try:
                exp_value = max(0, int(record.get("exp", 0) or 0))
            except (TypeError, ValueError):
                exp_value = 0
            per_companion = claimed_rewards.setdefault(str(companion_id), {})
            if not isinstance(per_companion, dict):
                claimed_rewards[str(companion_id)] = per_companion = {}
            for level in STAR_REWARD_LEVELS:
                if exp_value >= self._star_threshold_exp(level):
                    per_companion.setdefault(str(level), True)
        ...
        pity_state = player.setdefault("draw_pity_state", {})
        if not isinstance(pity_state, dict):
            pity_state = player["draw_pity_state"] = {}
        self._normalize_draw_state(pity_state, has_companion)

        pity_states = player.get("draw_pity_states", {})
        for scope_states in pity_states.values() if isinstance(pity_states, dict) else ():
            if not isinstance(scope_states, dict):
                continue
            for state in scope_states.values():
    ...
    def _draw_state_for(self, player: Dict[str, Any], scope_id: str, pool_id: Optional[str] = None) -> Dict[str, Any]:
        """Return one player-wide guarantee state, independent of group/pool."""
        state = player.setdefault("draw_pity_state", {})
        if not isinstance(state, dict):
            state = player["draw_pity_state"] = {}
        if not state and isinstance(player.get("draw_state"), dict):
            state.update(copy.deepcopy(player["draw_state"]))
        self._normalize_draw_state(state, bool(player.get("npcs")))
        # Keep the legacy mirror for old integrations and display code.
    ...
    def _owned_exclusive_item_names(self, player: Dict[str, Any], companion_id: str) -> List[str]:
        ...
        if not isinstance(owned, dict):
            return []
        names = self._exclusive_item_names(self._character_or_none(companion_id))
        return [names[int(slot)] for slot in owned if str(slot).isdigit() and int(slot) < len(names)]

    @staticmethod
    def _owned_exclusive_slots(player: Dict[str, Any], companion_id: str) -> set[int]:
        owned = player.get("exclusive_items", {}).get(companion_id, {})
        if not isinstance(owned, dict):
            return set()
        return {int(slot) for slot in owned if str(slot).isdigit() and int(slot) >= 0}
    ...
    def _grant_exclusive_item(
        ...
        if requested and requested not in available:
            return False
        next_slot = next((index for index in range(len(available)) if str(index) not in owned), None)
        if next_slot is None:
            return False
        # Every source of exclusive items uses the same stable slot order.
        # An explicitly selected name is accepted only when it is the next
        # unowned slot; this prevents the WebUI grant dropdown from skipping
        # earlier items.  Names can still be edited in-place without changing
        # the saved slot ownership.
        if requested:
            try:
                if available.index(requested) != next_slot:
                    return False
            except ValueError:
                return False
        target_slot = next_slot
        owned[str(target_slot)] = {"owned_at": datetime.now().strftime("%Y-%m-%d")}
        return True
    ...
    def _has_exclusive_item(
        ...
        if exclusive_name:
            names = self._exclusive_item_names(self._character_or_none(companion_id))
            try:
                return str(names.index(exclusive_name)) in owned
            except ValueError:
                return False
        return bool(owned)
    ...
    @staticmethod
    def _star_threshold_exp(level: int) -> int:
        index = max(1, min(len(LEVEL_REQUIREMENTS), int(level)))
        return sum(LEVEL_REQUIREMENTS[:index - 1])

    def _process_star_rewards(self, player: Dict[str, Any], companion_id: str, before: int, after: int) -> List[str]:
        companion = self._character_or_none(companion_id)
        if not companion or companion.get("kind") != COMPANION_KIND:
            return []
        claimed_all = player.setdefault("star_rewards_claimed", {})
        if not isinstance(claimed_all, dict):
            player["star_rewards_claimed"] = claimed_all = {}
        claimed = claimed_all.setdefault(companion_id, {})
        if not isinstance(claimed, dict):
            claimed_all[companion_id] = claimed = {}
        configured = self._normalize_star_rewards(companion.get("star_rewards"))
        messages: List[str] = []
        for level in STAR_REWARD_LEVELS:
            key = str(level)
            threshold = self._star_threshold_exp(level)
            if before >= threshold or after < threshold or claimed.get(key):
                continue
            reward = configured.get(key, {})
            coins = max(0, int(reward.get("coins", 0) or 0))
            if coins:
                player["coins"] = max(0, int(player.get("coins", 0) or 0)) + coins
                messages.append(f"{level}星 +{coins} 星币")
            if reward.get("exclusive"):
                # Rewards follow the currently equipped companion, as agreed;
                # if it is invalid after a migration, fall back to the levelled
                # companion so the milestone still has deterministic behavior.
                target_id = str(player.get("current_npc") or companion_id)
                target = self._character_or_none(target_id)
                if not target or target.get("kind") != COMPANION_KIND or target_id not in player.get("npcs", {}):
                    target_id = companion_id
                names = self._exclusive_item_names(self._character_or_none(target_id))
                owned_before = self._owned_exclusive_slots(player, target_id)
                next_slot = next((index for index in range(len(names)) if index not in owned_before), None)
                if next_slot is not None and self._grant_exclusive_item(player, target_id):
                    granted_name = names[next_slot] if next_slot < len(names) else "专属物品"
                    messages.append(f"{level}星获得专属物品：{granted_name}")
                else:
                    messages.append(f"{level}星专属物品已标记（无可发放槽位）")
            claimed[key] = True
        return messages
    ...
    def _add_exp(self, player: Dict[str, Any], character_id: str, exp: int) -> int:
        ...
        self._process_star_rewards(player, character_id, before, record["exp"])
        return granted
    ...
    def _arg_text(self, event: AstrMessageEvent) -> str:
        ...
        command_prefixes = [
            "剧本杀帮助", "同伴帮助", "伙伴帮助", "赠送星币", "发放星币", "扣除星币", "扣星币", "扣除星币数", "赠送同伴", "发放同伴", "赠送角色", "赠送NPC", "赠送皮肤", "赠送专属", "发放专属", "赠送专属物品", "发放道具", "发放权限", "取消权限", "购买",
            "切换同伴", "切换角色", "更换角色", "选择角色", "NPC信息", "npc信息", "查询NPC",
            "角色信息", "每日打卡", "我的星币", "NPC仓库", "npc仓库", "我的NPC",
            "状态栏", "角色状态", "抽奖", "npc抽奖", "NPC抽奖", "星币", "钱包",
            "中奖号码", "开奖", "每日挖矿", "挖矿领取", "挖矿", "打卡", "查NPC", "同伴栏", "物品栏", "道具栏", "我的道具", "道具仓库", "使用道具", "使用", "出售道具", "出售", "赠送道具", "赠送",
        ]
    ...
    def _find_item_exact(self, value: str) -> Optional[Dict[str, Any]]:
        """Resolve a traded item only by its stable ID or complete displayed name."""
        normalized = str(value or "").strip().lower()
        if not normalized:
            return None
        for item in self._items():
            candidates = (item.get("id"), item.get("name"), item.get("english_name"))
            if any(normalized == str(candidate or "").strip().lower() for candidate in candidates):
                return item
        return None

    def _find_buyable_entry_exact(self, value: str) -> Optional[Dict[str, Any]]:
        normalized = str(value or "").strip().lower()
        if not normalized:
            return None
        matches = []
        for entry in self.characters:
            if entry.get("kind") not in {COMPANION_KIND, SKIN_KIND, ITEM_KIND}:
                continue
            candidates = (entry.get("id"), entry.get("name"), entry.get("english_name"), entry.get("skin"))
            if any(normalized == str(candidate or "").strip().lower() for candidate in candidates):
                matches.append(entry)
        return matches[0] if len(matches) == 1 else None

    def _parse_purchase_command(self, event: AstrMessageEvent) -> Tuple[Optional[Dict[str, Any]], Optional[int]]:
        argument = self._arg_text(event)
        matched = re.fullmatch(r"(.+?)\s+(\d+)", argument)
        if not matched:
            return None, None
        name, amount_text = matched.groups()
        amount = self._parse_trade_amount(amount_text)
        return self._find_buyable_entry_exact(name), amount

    @staticmethod
    def _strip_mention_tokens(value: str) -> str:
        """Remove common rendered @ tokens while identity still comes from the message segment."""
        value = re.sub(r"\[CQ:at,[^\]]+\]", " ", value or "")
        value = re.sub(r"<@!?\d+>", " ", value)
        value = re.sub(r"@\S+", " ", value)
        return re.sub(r"\s+", " ", value).strip()

    def _parse_item_amount_command(
        self,
        event: AstrMessageEvent,
        command: str,
        requires_target: bool = False,
    ) -> Tuple[Optional[Dict[str, Any]], Optional[int]]:
        """Parse item trades; sales additionally support `出售道具名数量`."""
        argument = self._arg_text(event)
        if requires_target:
            argument = self._strip_mention_tokens(argument)
        matched = re.fullmatch(r"(.+?)\s+(\d+)", argument)
        if matched:
            item_name, amount_text = matched.groups()
            amount = self._parse_trade_amount(amount_text)
            return self._find_item_exact(item_name), amount

        # Only player sales allow the compact form.  Gifting deliberately keeps
        # its native-mention layout unchanged so target IDs cannot be mistaken
        # for an item quantity.
        if command != "出售" or requires_target:
            return None, None

        normalized_argument = argument.strip().lower()
        matches: Dict[Tuple[str, int], Dict[str, Any]] = {}
        for item in self._items():
            for candidate in (item.get("id"), item.get("name"), item.get("english_name")):
                candidate_text = str(candidate or "").strip()
                normalized_candidate = candidate_text.lower()
                if not candidate_text or not normalized_argument.startswith(normalized_candidate):
                    continue
                amount = self._parse_trade_amount(argument[len(candidate_text):].strip())
                if amount is not None:
                    matches[(str(item.get("id") or ""), amount)] = item
        if len(matches) != 1:
            return None, None
        (item_id, amount), item = next(iter(matches.items()))
        return item, amount

    @staticmethod
    def _parse_trade_amount(value: str) -> Optional[int]:
        if not re.fullmatch(r"\d+", str(value or "")):
            return None
        amount = int(value)
        return amount if 1 <= amount <= 9999 else None

    def _is_player_item_gift_message(self, event: AstrMessageEvent, text: str) -> bool:
        """Reserve generic `赠送` only for a native-mention player item transfer."""
        admin_prefixes = ("赠送星币", "赠送同伴", "赠送角色", "赠送NPC", "赠送皮肤", "赠送专属", "赠送专属物品")
        return (
            text.startswith("赠送")
            and not text.startswith(admin_prefixes)
            and self._extract_at_id(event) is not None
        )
    ...
    def _parse_page(self, event: AstrMessageEvent, max_page: int = 999) -> int:
        return self._parse_page_text(self._arg_text(event), max_page)

    def _parse_page_text(self, text: str, max_page: int = 999) -> int:
    ...
    def _is_inventory_argument(self, value: str) -> bool:
        """Accept an optional @target QQ followed by an optional page number."""
        text = (value or "").strip()
        if self._is_page_argument(text):
            return True
        text = re.sub(r"\[CQ:at,qq=\d+[^\]]*\]", "@target", text, flags=re.I)
        return bool(
            re.fullmatch(
                r"@?(?:\d{5,12}|[^\s@]+)(?:\s*(?:第?\s*(?:\d+|[零一二三四五六七八九十百千两]+)\s*页?)?)?",
                text,
            )
        )

    def _inventory_view_target(self, event: AstrMessageEvent) -> Tuple[Optional[Dict[str, Any]], int]:
        """Resolve `同伴栏 @QQ [页码]` without creating a player record."""
        target_id = self._extract_at_id(event)
        argument = self._arg_text(event)
        remaining = re.sub(r"\[CQ:at,qq=\d+[^\]]*\]", " ", argument, flags=re.I).strip()
        if target_id:
            remaining = re.sub(r"^@?\S+", "", remaining).strip()
        elif remaining:
            match = re.match(r"@?(\d{5,12})(?:\s+|$)(.*)", remaining)
            if match:
                target_id, remaining = match.group(1), match.group(2).strip()
        page = self._parse_page_text(remaining)
        if not target_id:
            return self._get_player(event), page
        return self._existing_player_by_scope(self._scope_id(event), target_id), page
    ...
    def _roll_draw(self, player: Dict[str, Any], scope_id: str = "global", pool_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        Pity is shared by the player across groups and pool rotations.  A
        companion/skin jackpot always resolves against the two entries that
        are currently enabled in the operator's pool.
        """
        current_id = str(player.get("current_npc") or "")
        if not current_id:
            companions = self._companions()
        ...
        if state["pity_count"] >= DRAW_PITY_TARGET and (companion_pool or skin_pool):
            state["pity_count"] = 0
            state["next_pity_kind"] = "random"
            return self._grant_featured_jackpot(player, companion_pool, skin_pool, "保底", current_id)
        ...
        if roll < cursor:
            if companion_pool:
                state["pity_count"] = 0
                state["next_pity_kind"] = "random"
                return self._grant_featured_jackpot(player, companion_pool, skin_pool, "大奖", current_id)
    ...
    def _grant_featured_jackpot(
        self,
        player: Dict[str, Any],
        companion_pool: List[Dict[str, Any]],
        skin_pool: List[Dict[str, Any]],
        source: str,
        current_id: str,
    ) -> Dict[str, Any]:
        """Grant the missing featured entry, then the featured companion's slots.

        The featured skin does not need to belong to the featured companion.
        Ownership is always checked against the player's shared library, never
        against their currently equipped companion.
        """
        companion = companion_pool[0] if companion_pool else None
        skin = skin_pool[0] if skin_pool else None
        missing = [entry for entry in (companion, skin) if entry and not self._owns_entry(player, entry)]
        if missing:
            entry = random.choice(missing)
            label = f"{source}{'同伴' if entry.get('kind') == COMPANION_KIND else '皮肤'}"
            return self._grant_draw_entry(player, entry, label)

        if companion:
            names = self._exclusive_item_names(companion)
            owned_slots = self._owned_exclusive_slots(player, companion["id"])
            next_slot = next((index for index in range(len(names)) if index not in owned_slots), None)
            if next_slot is not None and self._grant_exclusive_item(player, companion["id"], names[next_slot]):
                return {
                    "kind": f"{source}专属物品",
                    "name": names[next_slot],
                    "exp": 0,
                    "character_id": companion["id"],
                    "entry_id": companion["id"],
                    "entry_kind": COMPANION_KIND,
                    "image": companion.get("image", ""),
                }

        # An exhausted or unconfigured exclusive list must still produce a
        # valid paid-draw result.  Keep the established duplicate conversion.
        current = self._character(current_id)
        exp = self._add_exp(player, current_id, 20) if current_id else 0
        return {
            "kind": f"{source}重复奖励转经验",
    ...
    def _ensure_fonts(self):
        ...
        for filename, url in DOWNLOADABLE_FONTS:
            ...
            try:
                ...
                self._download_font(url, tmp_path)
                tmp_path.replace(path)
            except Exception as exc:
                tmp_path.unlink(missing_ok=True)
                # Font access is optional for startup. Keep existing operator
                # files untouched and let Pillow/system fallback fonts render
                # while a later restart can retry the download.
                logger.warning(f"下载中文字体失败：{filename}，插件将继续启动并使用回退字体：{exc}")
                break

    def _seed_bundled_fonts(self) -> None:
        """Copy packaged CJK fonts only when the persistent copy is missing."""
        bundled_dir = self.legacy_data_dir / "fonts"
        if not bundled_dir.is_dir():
            logger.warning(f"插件包未找到内置字体目录：{bundled_dir}")
            return
        self.font_dir.mkdir(parents=True, exist_ok=True)
        copied = 0
        for source in bundled_dir.iterdir():
            if not source.is_file() or source.suffix.lower() not in {".otf", ".ttf", ".ttc"}:
                continue
            destination = self.font_dir / source.name
            # A failed download can leave a zero-byte or truncated file in
            # the persistent directory. Treat that as missing so the bundled
            # font repairs it on the next startup; keep valid operator fonts.
            if not destination.exists() or destination.stat().st_size <= 8 * 1024 * 1024:
                shutil.copy2(source, destination)
                copied += 1
        if copied:
            logger.info(f"已从插件包安装 {copied} 个中文字体文件")

    @staticmethod
    def _download_font(url: str, destination: Path) -> None:
        request = urllib.request.Request(url, headers={"User-Agent": "astrbot-plugin-juben-npc/2.5.13"})
        with urllib.request.urlopen(request, timeout=FONT_DOWNLOAD_TIMEOUT_SECONDS) as response:
            ...
            if content_length:
                try:
                    ...
                    content_length = None
                if content_length is not None and content_length > FONT_DOWNLOAD_MAX_BYTES:
                    raise ValueError("字体文件超过 32MB 限制")
            total = 0
            with destination.open("wb") as output:
                while True:
                    ...
                    if not chunk:
                        break
                    total += len(chunk)
                    if total > FONT_DOWNLOAD_MAX_BYTES:
                        raise ValueError("字体文件超过 32MB 限制")
                    output.write(chunk)
        if total <= 8 * 1024 * 1024:
            raise ValueError("字体文件下载不完整")
    ...
    def _font(self, size: int, bold: bool = False, family: str = "default") -> ImageFont.FreeTypeFont:
        ...
        if font_path:
            ...
            return self._cache_font(cache_key, font)
    ...
    def _cache_font(self, cache_key: Tuple[int, bool, str], font: ImageFont.FreeTypeFont) -> ImageFont.FreeTypeFont:
        while len(self._font_cache) >= FONT_CACHE_MAX_SIZE:
    ...
    def _save_rendered_image(self, image: Image.Image, path: Path) -> Path:
        """Persist a disposable reply image, release its pixels, then maintain the cache."""
        self.render_dir.mkdir(parents=True, exist_ok=True)
        try:
            image.save(path)
        finally:
    ...
    def _prune_render_cache(self, force: bool = False) -> None:
        """Prune only generated reply PNGs; permanent uploaded assets live elsewhere."""
        if not force:
            self._render_operations = getattr(self, "_render_operations", 0) + 1
            if self._render_operations < RENDER_CACHE_PRUNE_EVERY:
                return
            self._render_operations = 0
        ...
        protected_seconds = RENDER_CACHE_PROTECTED_AGE.total_seconds()
        expired_seconds = RENDER_CACHE_MAX_AGE.total_seconds()

        def cache_files() -> List[Path]:
            try:
                return [path for path in self.render_dir.iterdir() if path.is_file() and path.suffix.lower() == ".png"]
            except OSError as exc:
                logger.warning("读取回复图片缓存失败：%s", exc)
                return []

        for path in cache_files():
            try:
                if now - path.stat().st_mtime >= expired_seconds:
                    path.unlink()
            except OSError as exc:
                logger.warning("清理过期回复图片失败：%s，原因：%s", path.name, exc)
        ...
        if total_size <= RENDER_CACHE_MAX_BYTES:
            return
        for modified_at, size, path in sorted(candidates):
            if total_size <= RENDER_CACHE_MAX_BYTES:
                break
            if now - modified_at < protected_seconds:
                continue
            try:
                path.unlink()
                total_size -= size
            except OSError as exc:
                logger.warning("清理回复图片缓存失败：%s，原因：%s", path.name, exc)
    ...
    def _render_text_card(self, title: str, lines: List[str], subtitle: str = "") -> Path:
        ...
        if subtitle:
            ...
            for wrapped in self._wrap(draw, subtitle, self._font(24), 850)[:2]:
                ...
                y += 32
        return self._save_rendered_image(img, path)
    ...
    def _render_inventory(self, player: Dict[str, Any], page: int = 1) -> Path:
        ...
        def companion_row_height(exclusive_items: List[str]) -> int:
            # Four item slots fit on the first row.  Extra operator-configured
            # slots continue on a second row instead of overlapping the skin.
            rows = (len(exclusive_items) + 3) // 4
        ...
        for _, companion, own_companion, owned_skins, exp, skin_offset, exclusive_items in groups:
            ...
            try:
                if not own_companion:
                    muted_portrait = ImageEnhance.Color(portrait).enhance(0.05).filter(ImageFilter.GaussianBlur(0.45))
                    portrait.close()
                    portrait = muted_portrait
            ...
            for item_index, exclusive_name in enumerate(exclusive_items):
                column, item_row = item_index % 4, item_index // 4
                item_x = 264 + column * 236
                ...
                draw.rounded_rectangle((item_x, item_y, item_x + 218, item_y + 24), radius=8, outline=item_border, width=1, fill=item_fill)
                label = f"专属物品：{exclusive_name}"
                draw.text((item_x + 8, item_y + 2), label[:13], font=self._font(13, True), fill=item_color)
    ...
    def _render_item_inventory(self, player: Dict[str, Any], page: int = 1) -> Path:
        ...
        for index, item in enumerate(entries):
            ...
            portrait = self._portrait(item, (226, 130))
            try:
                img.alpha_composite(portrait, (x + 17, y + 17))
            finally:
                portrait.close()
    ...
    def _render_npc_info(self, player: Dict[str, Any], character: Dict[str, Any]) -> Path:
        ...
        draw = ImageDraw.Draw(img)
        portrait = self._portrait(character, (430, 650))
        try:
            img.alpha_composite(portrait, (45, 70))
        finally:
            portrait.close()
